...
<div class="product product__card col-lg-4 col-sm-6 <?= !$offer['CAN_BUY'] ? 'disabled' : '' ?>"
  ...
  <div class="product__card-overlay">
    ...
    <div class="proudct__card-cart">
      ...
      <?php } else { // $offer['CAN_BUY'] === false ?>
        <?php // заглушка для js ?>
        <div class="product__card-price" data-entity="price-block">
              <span class="product__card-price old js-old-price" id="<?= $itemIds['PRICE_OLD'] ?>"></span>
          <span class="product__card-price product__card-price--new js-price" id="<?= $itemIds['PRICE'] ?>"></span>
        </div>
        <div class="product__card-button">
          <button disabled class="button bg-blue-1 jsAddToCart" data-offer-id="<?= $offer['ID'] ?>">
            Нет в наличии
          </button>
        </div>
      <?php } ?>
