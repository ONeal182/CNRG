					...
					<? if ($arParams["DISPLAY_PICTURE"] != "N") : ?>
						...
						?>
							<div class="mb-5 news-detail-img">
								<img class="card-img-top" src="<?= $arResult["SLIDER"][0]["SRC"] ?>" alt="<?= $arResult["SLIDER"][0]["ALT"] ?>" title="<?= $arResult["SLIDER"][0]["TITLE"] ?>" />
							</div>
						<?
						} else if (is_array($arResult["DETAIL_PICTURE"])) {
						?>
							<div class="news-page__img-background" style="background-image:url(<?= $arResult["DETAIL_PICTURE"]["SRC"] ?>)"></div>
						<?
						}
						?>
					<? endif ?>
